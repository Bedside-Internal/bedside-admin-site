...
function EnvironmentBadge({ env }: { env: string }) {
    let styles = "bg-sand text-ink/50"; // Default fallback

    const lowerEnv = env.toLowerCase();
    if (lowerEnv === "production" || lowerEnv === "prod") {
        styles = "bg-coral/15 text-coral";
    } else if (lowerEnv === "staging" || lowerEnv === "stage") {
        styles = "bg-amber/15 text-amber";
    }

    return (
        <span
            className={`ml-2 inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-semibold leading-tight capitalize ${styles}`}
        >
            {env}
        </span>
    );
                    ...
                    {users.map((user) => {
                        ...
                        return (
                            <tr
                                ...
                                className={`cursor-pointer border-b border-ink/5 transition-colors ${isSelected
                                        ? "bg-mint/8"
                                        : "hover:bg-sand/60"
                                    }`}
                            >
                                <td className="py-3 pr-4 pl-6 font-medium text-ink">
                                    {user.email}
                                    {user.environment && <EnvironmentBadge env={user.environment} />}
                                </td>
