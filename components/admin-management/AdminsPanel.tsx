...
import { listAdmins, listRoles, grantAdminAccess, revokeAdminAccess } from "@/lib/api/admin";
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [adminsData, rolesData] = await Promise.all([
        listAdmins(token),
        listRoles(token), // Needed for the modal dropdown
      ]);
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="mt-1 text-xs text-ink/40">
                Grant console access to a user to get started.
              </p>
            </div>
          ) : (
            <ul className="divide-y divide-ink/5">
              {admins.map((admin) => (
                <li
                  key={admin.id}
                  className="group flex items-center justify-between px-5 py-4 transition hover:bg-sand/40"
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {admin.permissionOverrides && (
                        <span className="shrink-0 rounded bg-amber/15 px-1.5 py-0.5 text-[10px] font-semibold text-amber">
                          Custom
                        </span>
                      )}
                    </div>
                    <div className="mt-1 flex items-center gap-3">
                      <span className="rounded-full bg-violet/10 px-2 py-0.5 text-xs font-medium text-violet">
                        {admin.roleLabel}
                      </span>
                  ...
                  {canDelete && (
                    <button
                      ...
                      className="ml-4 shrink-0 rounded-lg p-1.5 text-ink/30 opacity-0 transition hover:bg-coral/10 hover:text-coral group-hover:opacity-100"
                      title="Revoke Access"
                    >
